...
5. 项目代码不在本地时的获取：私有仓库的 clone 让用户在系统终端完成，AI 不要反复试各种拉取姿势
6. 被沙盒或权限拦住的任何操作：**绝不尝试绕过**（不提权、不改参数钻洞、不开子代理绕）。停下来，让用户换系统终端跑，或把正确的文件夹开成工作区。沙盒里 gh 报 "token in keyring is invalid" 多半是读不到系统钥匙串的假象——先让用户在系统终端 `gh auth status` 验证，别引导用户重新登录。详见 gotchas「沙盒 IDE」一节
