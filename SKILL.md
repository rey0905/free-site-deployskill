...
5. 项目代码不在本地时的获取：私有仓库的 clone 让用户在系统终端完成，AI 不要反复试各种拉取姿势。目录里已有可用副本就直接用，**绝不清空重建**
6. 被沙盒或权限拦住的任何操作：**绝不暗中绕过**（不提权、不改参数钻洞、不开子代理绕）。停下来，让用户换系统终端跑，或把正确的文件夹开成工作区。通过 IDE 自身的权限对话框向用户申请更高权限（弹窗、用户明确点同意的那种）是合法正路，不算绕过。沙盒里 gh 报 "token in keyring is invalid" 多半是读不到系统钥匙串的假象——先让用户在系统终端 `gh auth status` 验证，别引导用户重新登录。详见 gotchas「沙盒 IDE」一节。
   **两次即停**：同一操作被拦两次，立即强制停止，不做第三次尝试——换工具、换参数、开子代理重跑都算"第三次尝试"。停下来原样报告三件事：卡在哪一步、被什么拦、用户需要在系统终端跑哪条命令。停下来交还给用户不是失败，是本 skill 定义的正确完成方式；空转找绕路才是失败
