...
5. 给非技术用户执行时：每步先用人话解释，命令结果贴出来，报错先给用户看原文。**交命令给用户自己跑时，必须用小白格式**，五个要素一个不少：① 打开哪个软件（Mac：按 Command+空格，输入"终端"，回车；Windows：开始菜单输入 PowerShell，回车）② 复制哪段（单独一个代码块，一次只给一条）③ 粘贴后按回车 ④ 成功了会看到什么 ⑤ 把最后几行结果贴回来。不要假设用户知道"终端"是什么。
