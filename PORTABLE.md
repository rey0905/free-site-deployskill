...
7. **被沙盒或权限拦住时绝不绕过**——不提权、不改参数钻洞、不开子代理绕。让用户把项目文件夹开成工作区，或把命令挪到系统终端跑。沙盒里 gh 报 "token in keyring is invalid" 且所有账号同时失效，多半是读不到系统钥匙串的假象：先让用户在系统终端 `gh auth status` 验证，别引导重新登录。
